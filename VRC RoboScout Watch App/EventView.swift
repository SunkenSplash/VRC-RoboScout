    ...
    var body: some View {
        VStack {
            if showLoading {
                ...
                Form {
                    Section("Event") {
                        NavigationLink(destination: EventInfoView(event: event).environmentObject(settings)) {
                            Text("Information")
                        }
                        NavigationLink(destination: EventTeamsView(event: event, teams_map: $teams_map, event_teams: $event_teams, event_teams_list: event_teams_list).environmentObject(settings)) {
                            Text("Teams")
                        }
                        if team != nil {
                            NavigationLink(destination: EventTeamMatchesView(teams_map: $teams_map, event: event, team: team!).environmentObject(settings)) {
